...
@date        : 2023-06-07
@file        : niqe.py
@description : Natural Image Quality Evaluator (NIQE) implementation: https://live.ece.utexas.edu/publications/2013/mittal2013.pdf
@version     : 1.0
"""
...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    img = img[:-hoffset, :] if hoffset > 0 else img
    img = img[:, :-woffset] if woffset > 0 else img

    img = img.astype(np.float32)
    im = Image.fromarray(img)
    size = tuple((np.array(im.size) * 0.5).astype(int))
    img2 = np.array(im.resize(size, Image.Resampling.BICUBIC))
...
def calculate_niqe(src_image: np.ndarray):
    """Computes Naturalness Image Quality Evaluator. [#f1]_ [#f2]

    Input a gray image of any quality and get back its distance from naturalness.

    Args:
        image (np.ndarray): gray image with [H, W]

    Returns:
        float: niqe score of source image
    ...
    References:
        ...
        [#f2] https://github.com/scikit-video/scikit-video/blob/master/skvideo/measure/niqe.py.
    ...
    # TODO: memoize
    params = scipy.io.loadmat(os.path.join(os.path.dirname(__file__), 'niqe_image_params.mat'))
    ...
    feats = get_patches_test_features(src_image, patch_size)
